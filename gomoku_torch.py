...
class Gomoku(GameState):
    ...
    def reset(self, current_player=1):
        ...
        self.nn_input = torch.zeros((4, w, w)).cuda()
        #self.nn_input = torch.zeros((4, w, w))
        self.nn_input[-1] = 1
    ...
    def clone(self):
        ...
        st.nn_input = torch.clone(self.nn_input)
        return st
    ...
    def move(self, action):
        ...
        # update nn_input
        self.nn_input = torch.zeros((4, self.w, self.w)).cuda()
        #self.nn_input = torch.zeros((4, self.w, self.w))
